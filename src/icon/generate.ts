...
export default async function generateIcon() {
  ...
  let iconBytesRead = '';
  try {
    iconBytesRead = fs.readFileSync(iconDirectory, 'utf-8');
  } catch (e) {
    return console.log('😱 读取 iconfont.json 失败');
  }
  ...
  let ohIconBytesRead = '';
  try {
    ohIconBytesRead = fs.readFileSync(ohIconDirectory, 'utf-8');
  } catch (e) {
    return console.log('😱 读取 oh_icon.dart 失败');
  }
