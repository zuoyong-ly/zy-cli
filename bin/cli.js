...
const {create, translate, generateEnum, dirTree, generateIcon, generateApiCode} = require("../lib");
...
program
	...
	.description("搜索文件")
	.action((projectName, options) => {
		dirTree(projectName, options);
	});
program
	.command("icon")
	.description("生成OhIcon")
	.action((projectName, options) => {
		generateIcon(projectName, options);
	});
...
program
	.command("apiCode [apis]")
	.description("生成填充接口文档内容")
	.action((apis, options) => {
		generateApiCode(apis, options);
	});
